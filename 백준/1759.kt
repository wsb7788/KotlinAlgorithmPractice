...
private fun find(depth:Int, temp:String, vowel:Int, constant:Int){
    ...
    for(i in depth until C){
        when(array[i]){
            'a', 'e', 'i', 'o', 'u'-> find(i+1,temp+array[i],vowel+1,constant)
            else -> find(i+1,temp+array[i],vowel,constant+1)
        }
    }
