...
private lateinit var choo: IntArray
fun main() = with(BufferedReader(InputStreamReader(System.`in`))){

    n = readLine().toInt()
    choo = readLine().split(" ").map { it.toInt() }.toIntArray()

    m = readLine().toInt()
    val check = readLine().split(" ").map { it.toInt() }.toIntArray()
    dp = Array(n+1){IntArray(31*500*2){-1} }
    dfs(0,0)

    repeat(m){
        if(check[it]>15000){
            print("N ")
        } else if(dp[n][check[it]] == 1){
            print("Y ")
        }else{
            print("N ")
        }
...
private fun dfs(index:Int, weight:Int){
    ...
    dfs(index+1, abs(weight - choo[index]))
    dfs(index+1, weight)
    dfs(index+1, abs(weight + choo[index]))
