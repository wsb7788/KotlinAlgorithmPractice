...
fun main()  = with(BufferedReader(InputStreamReader(System.`in`))) {
    ...
    for(i in 0 until n){
        ...
        maxHeight = max(array[i].maxOf { it },maxHeight)
    }
...
private fun expend(X:Int, Y:Int, height: Int){
    for(b in 0 until 4){
        val nextX = X + dx[b]
        val nextY = Y + dy[b]
        if(nextX<0 || nextX >= n || nextY<0||nextY>=n) continue
        if(array[nextX][nextY] > height && !visit[nextX][nextY]){
            visit[nextX][nextY] = true
            expend(nextX,nextY,height)
        }
    }
}
