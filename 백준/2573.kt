...
fun main() = with(BufferedReader(InputStreamReader(System.`in`))){
    ...
    for(i in 0 until n){
        ...
        temp2.forEachIndexed { index, value ->
            if(value>0) q.add(Pair(i,index))
        }
        array[i] = temp2
    }
    ...
    while(q.isNotEmpty()){
        ...
        while(temp.isNotEmpty()){
            ...
            var tempcount = 0
            for(i in 0 until 4){
                val xx = aa.first + dx[i]
                val yy = aa.second + dy[i]
                if(!(xx in 0 until n && yy in 0 until m)) continue
                if(array[xx][yy] == 0 && !visit[xx][yy]) tempcount++
            }
            if(array[aa.first][aa.second]<=tempcount) array[aa.first][aa.second] = 0
            else {
                array[aa.first][aa.second] -= tempcount
                q.add(Pair(aa.first,aa.second))
            }
        }
...
private fun bfs(row:Int, col:Int){
    for(i in 0 until 4){
        ...
        val yy = col + dy[i]
        if(!(xx in 0 until n && yy in 0 until m)) continue
        if(visit[xx][yy]||array[xx][yy]==0) continue
        visit[xx][yy] = true
        bfs(xx,yy)
    }
